/**
 * 图谱直接调用接口：关系图。
 * POST /api/tools/echart/graph
 * 请求体与 doc/echart-mcp-server-architecture.md 4.1 一致；通过 MCP SDK 调用 ECharts MCP 的 generate_graph_chart。
 * 响应：outputType=option 时 { type: "option", option }，png/svg 时 { type: "image", data, mimeType }。
 */
...
export async function POST(request: Request) {
  ...
  try {
    const result = await callEchartMcpTool(url, "generate_graph_chart", body);
